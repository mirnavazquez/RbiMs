...
read_dbcan3 <- function(dbcan_path,
  ...
  lapply_read_delim_bind_rows <-
    function(path, pattern = "*overview.txt") {
      files = list.files(path, pattern, full.names = TRUE)
      lapply(files, read.delim, check.names = F) %>% bind_rows()
    }
  dbcan_df <-
    suppressWarnings(lapply_read_delim_bind_rows(ruta_dbcan))
  # Reading data ----------------------------------------------------------####
  dbcan_df_format <- suppressWarnings(
    dbcan_df %>%
      ...
      mutate(
        dbCAN = case_when(
          str_detect(.data$dbCAN_names, "CBM") ~
      ...
      dplyr::select(-.data$Scaffold_name)
                 ...
                 .data$dbCAN,
                 .data$domain_name,
                 .data$Signalp) %>% summarise_if(is.numeric, sum) #juntando
