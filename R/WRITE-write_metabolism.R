...
write_metabolism <- function(data_interpro = NULL,
                             data_ko = NULL) {
  # Read the interpro -------------------------------------------------------####
  databases_names <-
    c(
  ...
  ko <- read_ko(data_ko) %>%
    ...
    ungroup()
  All_data <- ko_mapped %>%
    pivot_longer(!all_of(data_to_select),
                 names_to = "Bin_name",
                 values_to = "Abundance") %>%
    select(-.data$Abundance) %>%
    distinct()
  ...
  main_metabolism <- map(data_to_select,
                         separate_kegg,
                         kegg_data = ko,
                         metabolism_data = All_data) %>%
    reduce(full_join, by = c("Bin_name", "Scaffold_name")) %>%
  ...
  # Fixing mapping ko -----------------------------------------------------####
  ko_mapped <- ko_mapped %>%
    mutate(across(all_of(data_to_select), ~ replace_na(.x, "---")))
  ko <- read_ko(data_ko)
  
  # Read the interpro -------------------------------------------------------####
  message("Creating the Metabolism book")
  Pfam <-
    read_interpro(data_interpro, database = "Pfam", profile = F) %>%
  ...
  INTERPRO <-
    read_interpro(data_interpro, database = "INTERPRO", profile = F) %>%
  ...
  TIGRFAM <-
    read_interpro(data_interpro, database = "TIGRFAM", profile = F) %>%
    rename(TIGRFAM_domain_name = .data$domain_name)  %>%
    mutate(Abundance = as.character(.data$Abundance))  %>%
    mutate(across(everything(), ~ replace_na(.x, "---"))) %>%
    mutate_at('Abundance', as.integer)
  SUPERFAMILY <-
    read_interpro(data_interpro, database = "SUPERFAMILY",
                  profile = F) %>%
  ...
  SMART <-
    read_interpro(data_interpro, database = "SMART", profile = F) %>%
    rename(SMART_domain_name = .data$domain_name)  %>%
    mutate(Abundance = as.character(.data$Abundance))  %>%
    mutate(across(everything(), ~ replace_na(.x, "---"))) %>%
    mutate_at('Abundance', as.integer)
  SFLD <-
    read_interpro(data_interpro, database = "SFLD", profile = F) %>%
  ...
  ProSiteProfiles <-
    read_interpro(data_interpro, database = "ProSiteProfiles",
                  profile = F) %>%
    ...
    mutate_at('Abundance', as.integer)
  ProSitePatterns <-
    read_interpro(data_interpro, database = "ProSitePatterns",
                  profile = F) %>%
  ...
  ProDom <-
    read_interpro(data_interpro, database = "ProDom", profile = F) %>%
    rename(ProDom_domain_name = .data$domain_name)   %>%
    mutate(Abundance = as.character(.data$Abundance))  %>%
    mutate(across(everything(), ~ replace_na(.x, "---"))) %>%
    mutate_at('Abundance', as.integer)
  PRINTS <-
    read_interpro(data_interpro, database = "PRINTS", profile = F) %>%
  ...
  PIRSF <-
    read_interpro(data_interpro, database = "PIRSF", profile = F) %>%
  ...
  Hamap <-
    read_interpro(data_interpro, database = "Hamap", profile = F) %>%
  ...
  Gene3D <-
    read_interpro(data_interpro, database = "Gene3D", profile = F) %>%
  ...
  Coils <-
    read_interpro(data_interpro, database = "Coils", profile = F) %>%
  ...
  CDD <-
    read_interpro(data_interpro, database = "CDD", profile = F) %>%
  ...
  list_of_datasets <-
    list(
      "MetabolismTable" = main_metabolism,
