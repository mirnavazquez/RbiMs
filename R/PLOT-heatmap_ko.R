...
heatmap_ko <- function(tibble_ko,
  ...
  tibble_ko_colnames_2 <- tibble_ko_colnames[tibble_ko_colnames %in%
                                               paths]
  tibble_ko_colnames_3 <-
    tibble_ko_colnames_2[!tibble_ko_colnames_2 %in%
                           y_axis_label]
  
  # Join ------------------------------------------------------------------####
  table_final <- tibble_ko_mod %>%
    pivot_wider(
      names_from = .data$Bin_name,
    ...
    select(-contains(".y")) %>%
    distinct() %>%
    rename_all(list(~ stringr::str_replace_all(., ".x", ""))) %>%
  ...
  # Order table -----------------------------------------------------------####
  if (quo_is_null(order_x_enquo) == F &&
      quo_is_null(order_y_enquo) == F) {
    sub_samp_ordered <- table_final[rownames(metabolism_order), ]
    sub_samp_ordered_2 <-
      sub_samp_ordered[, rownames(experiment_order)]
  }
  if (quo_is_null(order_x_enquo) == T &&
      quo_is_null(order_y_enquo) == F) {
    sub_samp_ordered <- table_final[rownames(metabolism_order), ]
  }
  if (quo_is_null(order_x_enquo) == F &&
      quo_is_null(order_y_enquo) == T) {
