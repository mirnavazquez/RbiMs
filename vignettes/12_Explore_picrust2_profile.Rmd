---
title: "12. Explore picrust2 profile"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{12. Explore_picrust2_profile}
...
```{r}
library(readr)
meta_path <- system.file("extdata", "Metadatos-MANGLAR.tsv", package = "rbims")
...
```{r}
ko_path <- system.file("extdata", "KO_metagenome_out", "KO_pred_metagenome_unstrat.tsv", package = "rbims")
stopifnot(ko_path != "")
KO_picrust2_profile_long <- read_picrust2(ko_path, database = "KO", profile = FALSE)
```
...
```{r}
ko_picrust2_pathway_map<-mapping_ko(KO_picrust2_profile_long)
```
...
```{r}
library(tidyr)
Energy_metabolisms<-ko_picrust2_pathway_map %>%
...
```{r}
library(tidyr)
specific_subset<-ko_picrust2_pathway_map%>%
  drop_na(Pathway) %>%
